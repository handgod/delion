					 ...
					 char *server_id)
{
    ...
    if ((sock = rfbConnect(rfbScreen, host, port)) < 0)
    {
    	rfbLog("rfbRepeaterConnection failed ! return NULL \n");
        return (rfbClientPtr)NULL;
    }
					 ...
					 char *server_id)
{
	...
	for (i = 0; i < 4; i++)
      ...
      if (!webSocketsCheck(cl)) {
    	...
    	rfbLog("rfbCloseClient No.1 \n ");
        rfbCloseClient(cl);
        rfbLog("rfbClientConnectionGone is called in rfbNewTCPOrUDPClient,!webSocketsCheck(cl)\n");
		...
		if (server_id != NULL)
		{
			...
			if (rfbWriteExact(cl, idString, 250) < 0) {
				...
				rfbLog("rfbCloseClient No.2 \n ");
				rfbCloseClient(cl);
		        rfbLog("rfbClientConnectionGone is called in rfbNewTCPOrUDPClient\n");
				rfbClientConnectionGone(cl);
				return NULL;
			}
			//sleep(2);
      ...
      if (rfbWriteExact(cl, pv, sz_rfbProtocolVersionMsg) < 0) {
        rfbLogPerror("rfbNewClient: write");
        rfbLog("rfbCloseClient No.3 \n ");
        rfbCloseClient(cl);
        rfbLog("rfbClientConnectionGone is called in rfbNewTCPOrUDPClient rfbWriteExact\n");
        rfbClientConnectionGone(cl);
        return NULL;
      }
    }
    ...
    case RFB_CLIENT_REFUSE:
    	rfbLog("rfbCloseClient No.4 \n ");
	    rfbCloseClient(cl);
	    rfbLog("rfbClientConnectionGone is called in %s,%s,%s\n",__FILE__,__LINE__,__func__);
	    rfbClientConnectionGone(cl);
	    cl = NULL;
	    break;
    }
...
rfbProcessClientMessage(rfbClientPtr cl)
{
    ...
    case RFB_PROTOCOL_VERSION:
        rfbLog("we will process message for RFB_PROTOCOL_VERSION");
        rfbProcessClientProtocolVersion(cl);
        return;
    case RFB_SECURITY_TYPE:
        rfbLog("we will process message for RFB_SECURITY_TYPE");
        rfbProcessClientSecurityType(cl);
        return;
    case RFB_AUTHENTICATION:
        rfbLog("we will process message for RFB_AUTHENTICATION");
        rfbAuthProcessClientMessage(cl);
        return;
    case RFB_INITIALISATION:
        rfbLog("we will process message for RFB_INITIALISATION");
    case RFB_INITIALISATION_SHARED:
       rfbLog("we will process message for RFB_INITIALISATION_SHARED");
        rfbProcessClientInitMessage(cl);
        return;
    default:
        rfbLog("we will process message for rfbProcessClientNormalMessage default");
...
#if 1 //20170315,don't close client,because from repeater version is RFB 000.000
	rfbLog("rfbCloseClient No.6 \n ");
	rfbCloseClient(cl);
...
rfbSendSupportedMessages(rfbClientPtr cl)
{
	...
	rfbSetBit(msgs.client2server, rfbExecuteShell);
	rfbSetBit(msgs.client2server, rfbLogcat);
	...
	rfbSetBit(msgs.server2client, rfbExecuteShell);
	rfbSetBit(msgs.server2client, rfbLogcat);
...
rfbBool
rfbSendExcuteShell(rfbClientPtr cl, char* cmd, int len, uint8_t ret){

	//const char* result = "shell";//runShell(cmd);
	char* shCmd = malloc(len + 1);
	memset(shCmd, 0, len + 1);
	memcpy(shCmd, cmd, len);
	LOGD("rfbSendExcuteShell:%s", shCmd);
	const char* result = jni_runShell(shCmd);
	rfbExcuteShellCmdMsg shmsg;
	
	shmsg.type = rfbExecuteShell;
	shmsg.ret = 0;
	shmsg.pad = 0;
	if(ret > 0 && result != NULL){
		int len = strlen(result);
		shmsg.length = Swap32IfLE(len);
		char* buf = malloc(sz_rfbExcuteShellCmdMsg + len);
		memcpy(buf, (char *)&shmsg, sz_rfbExcuteShellCmdMsg);
		memcpy(buf + sz_rfbExcuteShellCmdMsg, result, len);
		LOCK(cl->sendMutex);
	    if (rfbWriteExact(cl, buf, sz_rfbExcuteShellCmdMsg + len) < 0) {
		  __android_log_print(ANDROID_LOG_INFO, "VNC", "write close client");
	      rfbLogPerror("rfbSendExcuteShell: write header");
	      rfbCloseClient(cl);
	    }
		UNLOCK(cl->sendMutex);
		free(buf);
		...
		shmsg.length = 0;
		LOCK(cl->sendMutex);
	    if (rfbWriteExact(cl, (char *)&shmsg, sz_rfbExcuteShellCmdMsg) < 0) {
	      rfbLogPerror("rfbSendExcuteShell: write");
	      rfbCloseClient(cl);
	    }
	    UNLOCK(cl->sendMutex);
	}
...
rfbBool
rfbSendLogcat(rfbClientPtr cl, char* param, int pLen){
	char* logcatSh = "logcat ";
	int cmdLen = strlen(logcatSh);
	char* shCmd = malloc(cmdLen + pLen + 1);
	memcpy(shCmd, logcatSh, cmdLen);
	memcpy(shCmd + cmdLen, param, pLen);
	shCmd[cmdLen + pLen] = 0;
	LOGD("rfbSendLogcat:%s", shCmd);

	char* result = jni_Logcat(shCmd);

	rfbLogcatMsg logmsg;
	
	logmsg.type = rfbLogcat;
	logmsg.compress = 1;
	logmsg.pad = 0;
	int len = 0;
	...
	logmsg.length = Swap32IfLE(len);
	if(result != NULL && len > 0){
		char* buf = malloc(sz_rfbLogcatMsg + len);
		memcpy(buf, (char *)&logmsg, sz_rfbLogcatMsg);
		memcpy(buf + sz_rfbLogcatMsg, result, len);
		LOCK(cl->sendMutex);
	    if (rfbWriteExact(cl, buf, sz_rfbLogcatMsg + len) < 0) {
	      rfbLogPerror("rfbSendLogcat: write");
	      rfbCloseClient(cl);
	    }
	    UNLOCK(cl->sendMutex);
		free(buf);
		...
		LOCK(cl->sendMutex);
	    if (rfbWriteExact(cl, (char *)&logmsg, sz_rfbLogcatMsg) < 0) {
	      rfbLogPerror("rfbSendLogcat: write");
	      rfbCloseClient(cl);
	    }
	    UNLOCK(cl->sendMutex);
...
rfbProcessClientNormalMessage(rfbClientPtr cl)
{
    ...
    if ((n = rfbReadExact(cl, (char *)&msg, 1)) <= 0) {
        if (n != 0)
            rfbLogPerror("rfbProcessClientNormalMessage:before switch read");
        rfbLog("rfbCloseClient No.21 rfbReadExact() return n=%d,0 means the other end has closed\n ",n);
#if 1
        rfbCloseClient(cl);
#endif
        return;
    ...
    case rfbSetPixelFormat:
        ...
        if ((n = rfbReadExact(cl, ((char *)&msg) + 1,
            ...
            if (n != 0)
                rfbLogPerror("rfbProcessClientNormalMessage:case rfbSetPixelFormat: read");
            rfbLog("rfbCloseClient No.22 \n ");
            rfbCloseClient(cl);
            return;
        }
    ...
    case rfbFixColourMapEntries:
        if ((n = rfbReadExact(cl, ((char *)&msg) + 1,
            ...
            if (n != 0)
                rfbLogPerror("rfbProcessClientNormalMessage:case rfbFixColourMapEntries: read");
            rfbLog("rfbCloseClient No.23 \n ");
            rfbCloseClient(cl);
            return;
        }
        rfbStatRecordMessageRcvd(cl, msg.type, sz_rfbSetPixelFormatMsg, sz_rfbSetPixelFormatMsg);
        rfbLog("rfbProcessClientNormalMessage:case rfbFixColourMapEntries: %s",
                "FixColourMapEntries unsupported\n");
#if 1  //temp removed
        rfbLog("rfbCloseClient No.24 \n ");
        rfbCloseClient(cl);
#endif
        return;
    ...
    case rfbSetEncodings:
    {
        ...
        if ((n = rfbReadExact(cl, ((char *)&msg) + 1,
            ...
            if (n != 0)
                rfbLogPerror("rfbProcessClientNormalMessage:case rfbSetEncodings: read");
            rfbLog("rfbCloseClient No.25 \n ");
            rfbCloseClient(cl);
            return;
        }
        ...
        for (i = 0; i < msg.se.nEncodings; i++) {
            if ((n = rfbReadExact(cl, (char *)&enc, 4)) <= 0) {
                if (n != 0)
                    rfbLogPerror("rfbProcessClientNormalMessage:case rfbSetEncodings:2 read");
                rfbLog("rfbCloseClient No.26 \n ");
                rfbCloseClient(cl);
                return;
            }
            enc = Swap32IfLE(enc);
...
#endif
		{
			...
			if(e == NULL) {
				...
				if(!handled)
					rfbLog("rfbProcessClientNormalMessage: case rfbSetEncodings:"
    ...
    case rfbFramebufferUpdateRequest:
    {
        ...
        if ((n = rfbReadExact(cl, ((char *)&msg) + 1,
            ...
            if (n != 0)
                rfbLogPerror("rfbProcessClientNormalMessage:case rfbFramebufferUpdateRequest: read");
            rfbLog("rfbCloseClient No.28 \n ");
            rfbCloseClient(cl);
            return;
        }
	...
	if ((n = rfbReadExact(cl, ((char *)&msg) + 1,
	    ...
	    if (n != 0)
		rfbLogPerror("rfbProcessClientNormalMessage:case rfbKeyEvent: read");
	    rfbLog("rfbCloseClient No.29 \n ");
	    rfbCloseClient(cl);
	    return;
	}
	...
	if ((n = rfbReadExact(cl, ((char *)&msg) + 1,
	    ...
	    if (n != 0)
		rfbLogPerror("rfbProcessClientNormalMessage:case rfbPointerEvent: read");
	    rfbLog("rfbCloseClient No.30 \n ");
	    rfbCloseClient(cl);
	    return;
	}
    ...
    case rfbFileTransfer:
        if ((n = rfbReadExact(cl, ((char *)&msg) + 1,
            ...
            if (n != 0)
                rfbLogPerror("rfbProcessClientNormalMessage:case rfbFileTransfer: read");
            rfbLog("rfbCloseClient No.31 \n ");
            rfbCloseClient(cl);
            return;
        }
    ...
    case rfbSetSW:
        if ((n = rfbReadExact(cl, ((char *)&msg) + 1,
                              sz_rfbSetSWMsg - 1)) <= 0) {
            if (n != 0)
                rfbLogPerror("rfbProcessClientNormalMessage: case rfbSetSW: read");
            rfbLog("rfbCloseClient No.32 \n ");
            rfbCloseClient(cl);
            return;
        }
    ...
    case rfbSetServerInput:
        if ((n = rfbReadExact(cl, ((char *)&msg) + 1,
            ...
            if (n != 0)
                rfbLogPerror("rfbProcessClientNormalMessage:case rfbSetServerInput: read");
            rfbLog("rfbCloseClient No.34 \n ");
            rfbCloseClient(cl);
            return;
        }
    ...
    case rfbTextChat:
        if ((n = rfbReadExact(cl, ((char *)&msg) + 1,
            ...
            if (n != 0)
                rfbLogPerror("rfbProcessClientNormalMessage: case rfbTextChat: read");
            rfbLog("rfbCloseClient No.35 \n ");
            rfbCloseClient(cl);
            return;
        }
        ...
        default:
            if ((msg.tc.length>0) && (msg.tc.length<rfbTextMaxSize))
            {
                ...
                if (str==NULL)
                {
                    ...
                    rfbLog("rfbCloseClient No.36 \n ");
                    rfbCloseClient(cl);
                    return;
                }
                if ((n = rfbReadExact(cl, str, msg.tc.length)) <= 0) {
                    if (n != 0)
                        rfbLogPerror("rfbProcessClientNormalMessage: default: read");
                    free(str);
                    rfbLog("rfbCloseClient No.37 \n ");
                    rfbCloseClient(cl);
                    return;
                }
	...
	if ((n = rfbReadExact(cl, ((char *)&msg) + 1,
	    ...
	    if (n != 0)
		rfbLogPerror("rfbProcessClientNormalMessage:case rfbClientCutText:1 read");
	    rfbLog("rfbCloseClient No.39 \n ");
	    rfbCloseClient(cl);
	    return;
	}
	...
	if ((n = rfbReadExact(cl, str, msg.cct.length)) <= 0) {
	    if (n != 0)
	        rfbLogPerror("rfbProcessClientNormalMessage:case rfbClientCutText:2 read");
	    free(str);
	    rfbLog("rfbCloseClient No.40 \n ");
	    rfbCloseClient(cl);
	    return;
	}
    ...
    case rfbPalmVNCSetScaleFactor:
      ...
      if ((n = rfbReadExact(cl, ((char *)&msg) + 1,
          ...
          if (n != 0)
            rfbLogPerror("rfbProcessClientNormalMessage:case rfbPalmVNCSetScaleFactor: read");
          rfbLog("rfbCloseClient No.41 \n ");
          rfbCloseClient(cl);
          return;
      }
    ...
    case rfbSetScale:
      ...
      if ((n = rfbReadExact(cl, ((char *)&msg) + 1,
          sz_rfbSetScaleMsg - 1)) <= 0) {
          if (n != 0)
            rfbLogPerror("rfbProcessClientNormalMessage: case rfbSetScale: read");
          rfbLog("rfbCloseClient No.42 \n ");
          rfbCloseClient(cl);
          return;
      }
      rfbStatRecordMessageRcvd(cl, msg.type, sz_rfbSetScaleMsg, sz_rfbSetScaleMsg);
    ...
    case rfbXvp:
      ...
      if ((n = rfbReadExact(cl, ((char *)&msg) + 1,
          sz_rfbXvpMsg - 1)) <= 0) {
          if (n != 0)
            rfbLogPerror("rfbProcessClientNormalMessage:case rfbXvp: read");
          rfbLog("rfbCloseClient No.43 \n ");
          rfbCloseClient(cl);
          return;
      }
	...
	case rfbExecuteShell:
		{
			__android_log_print(ANDROID_LOG_INFO, "VNC", "rfbExecuteShell");
			if ((n = rfbReadExact(cl, ((char *)&msg) + 1,
                           sz_rfbExcuteShellCmdMsg - 1)) <= 0) {
                __android_log_print(ANDROID_LOG_INFO, "VNC", "rfbReadExact:%d", n);
	            if (n != 0)
	                rfbLogPerror("rfbProcessClientNormalMessage: read");
	            rfbCloseClient(cl);
			}
			int len = Swap32IfLE(msg.shcmd.length);
			LOGD("rfbExecuteShell len:%d", len);
			LOGD("rfbExecuteShell msg.shcmd.length:%d", msg.shcmd.length);
			__android_log_print(ANDROID_LOG_INFO, "VNC", "msg.shcmd.length:%d", msg.shcmd.length);
			str = (char *)malloc(len);
			if ((n = rfbReadExact(cl, (char*)str, len)) <= 0) {
				__android_log_print(ANDROID_LOG_INFO, "VNC", "rfbReadExact str:%d", n);
				if (n != 0)
					rfbLogPerror("rfbProcessClientNormalMessage: read");
				free(str);
				rfbCloseClient(cl);
			...
			__android_log_print(ANDROID_LOG_INFO, "VNC", "rfbStatRecordMessageRcvd");
			rfbStatRecordMessageRcvd(cl, msg.type, sz_rfbExcuteShellCmdMsg+len, sz_rfbExcuteShellCmdMsg+len);
			__android_log_print(ANDROID_LOG_INFO, "VNC", "rfbSendExcuteShell");
			rfbSendExcuteShell(cl, str, len, msg.shcmd.ret);
			__android_log_print(ANDROID_LOG_INFO, "VNC", "rfbSendExcuteShell end");
			free(str);
        }
		return;
	case rfbLogcat:
		{
			...
			int len = Swap32IfLE(msg.logcat.length);
			str = (char *)malloc(msg.logcat.length);
			if ((n = rfbReadExact(cl, str, len)) <= 0) {
				if (n != 0)
					rfbLogPerror("rfbProcessClientNormalMessage: read");
				free(str);
				rfbCloseClient(cl);
			...
			rfbStatRecordMessageRcvd(cl, msg.type, sz_rfbLogcatMsg+len, sz_rfbLogcatMsg+len);
			rfbSendLogcat(cl, str, len);
			free(str);
		}
		return;
    default:
	{
	    ...
	    rfbLog("rfbProcessClientNormalMessage:  default: unknown message type %d\n",
		    msg.type);
#if 0
	    rfbLog(" ... closing connection\n");
	    rfbLog("rfbCloseClient No.44 \n ");
	    rfbCloseClient(cl);
#else
	    rfbLog(" ... don't close connection for test!\n");
#endif
