...
clientOutput(void *data)
{
    ...
    while (1) {
		...
		if (cl->sock == -1) {
			...
			rfbLog("clientOutput thread,cl->sock == -1,Client has disconnected. \n");
			return NULL;
		}
		if (cl->state != RFB_NORMAL || cl->onHold) {
			/* just sleep until things get normal */
	//		rfbLog("clientOutput thread,cl->state != RFB_NORMAL || cl->onHold,just sleep until things get normal. \n");
		...
		if (sraRgnEmpty(cl->requestedRegion)) {
			...
			rfbLog("clientOutput thread always require a FB Update Request (otherwise can crash.)\n");
		} else {
			...
			if(!haveUpdate) {
				...
				rfbLog("clientOutput thread haveUpdate == FALSE ,no update \n");
			}
		}
        ...
        rfbIncrClientRef(cl);
        LOCK(cl->sendMutex);
        rfbLog("clientOutput thread Now actually send the update. \n");
...
while (1) {
	...
	if (cl->sock == -1) {
	  /* Client has disconnected. */
		rfbLog("clientInput cl->sock == -1,Client has disconnected,go to rfbClientConnectionGone \n");
            break;
        }
	...
	tv.tv_usec = 0;
	//cindy change wait time o
  n = select(cl->sock + 1, &rfds, &wfds, &efds, &tv);
  //n = select(cl->sock + 1, &rfds, &wfds, &efds, NULL);
  //end change
	rfbLog("clientInput is called in clientInput , select(),sock=%d,return=%d \n",cl->sock,n);

	if (n < 0) {
	    rfbLogPerror("clientInput ReadExact: select");
			rfbLog("clientInput is called in clientInput , select()return <0 \n");
	    break;
	}
	if (n == 0) /* timeout */
	{
		rfbLog("clientInput select(),sock=%d,return=%d,timeout!! \n",cl->sock,n);
        ...
        {
        	rfbLog("clientInput select() return=%d,cl->sock=%d,rfbSendFileTransferChunk is called!\n",n,cl->sock);
            rfbSendFileTransferChunk(cl);
        }
        if (FD_ISSET(cl->sock, &rfds) || FD_ISSET(cl->sock, &efds))
        {
           	rfbLog("clientInput select() return=%d,cl->sock=%d,state is = %d,rfbProcessClientMessage is called!\n",n,cl->sock,cl->state);
            //cindy comment
    ...
    rfbLog("rfbClientConnectionGone is called in clientInput \n");
...
{
	rfbLog("rfbCloseClient No.58 \n ");
    rfbCloseClient(cl);
    rfbLog("rfbClientConnectionGone is called in rfbRefuseOnHoldClient \n");
    rfbClientConnectionGone(cl);
}
...
void rfbScreenCleanup(rfbScreenInfoPtr screen)
{
  ...
  while(cl1) {
    ...
    rfbLog("rfbClientConnectionGone is called in rfbScreenCleanup \n");
