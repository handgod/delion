...
int rfbSendNewScaleSize(rfbClientPtr cl)
{
    ...
    if (cl->PalmVNC==TRUE)
    {
        ...
        if (rfbWriteExact(cl, (char *)&pmsg, sz_rfbPalmVNCReSizeFrameBufferMsg) < 0) {
            ...
            rfbLog("rfbCloseClient No.50 \n ");
            rfbCloseClient(cl);
    	    rfbLog("rfbClientConnectionGone is called in rfbSendNewScaleSize,cl->PalmVNC==TRUE \n");
    ...
    else
    {
        ...
        if (rfbWriteExact(cl, (char *)&rmsg, sz_rfbResizeFrameBufferMsg) < 0) {
            rfbLogPerror("rfbNewClient: write");
            rfbLog("rfbCloseClient No.51 \n ");
            rfbCloseClient(cl);
    	    rfbLog("rfbClientConnectionGone is called in rfbSendNewScaleSize,cl->PalmVNC==FALSE \n");
            rfbClientConnectionGone(cl);
            return FALSE;
        }
    }
