...
rfbVncAuthNone(rfbClientPtr cl)
{
    ...
    if (cl->protocolMajorVersion==3 && cl->protocolMinorVersion > 7 && cl->protocolMinorVersion != 889) {
        ...
        if (rfbWriteExact(cl, (char *)&authResult, 4) < 0) {
            ...
            rfbLog("rfbCloseClient No.61 \n ");
            rfbCloseClient(cl);
            return;
        }
    }
    cl->state = cl->protocolMinorVersion == 889 ? RFB_INITIALISATION_SHARED : RFB_INITIALISATION;
    rfbLog("rfbVncAuthNone rfbProcessClientSecurityType state is = %d \n",cl->state);
...
#define MAX_SECURITY_TYPES 255
    ...
    switch (primaryType) {
    case rfbSecTypeNone:
        rfbLog("rfbSendSecurityTypeList rfbSecTypeNone");
...
rfbAuthNewClient(rfbClientPtr cl)
{
    ...
    rfbLog("rfbAuthNewClient set securityType fbSecTypeNone");
