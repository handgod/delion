...
#ifdef SUPPORT_INJECT_EVENT

void initInput(int width, int height)
{
	L("---Initializing inputevent_socket...---\n");
	//return ;
}

int send_event_to_app(char *send_buffer)
{
	int inputevent_socket, n;
	unsigned int length;
	struct sockaddr_in inputevent_server;
	char temp_buffer[1024];

	inputevent_socket = socket(AF_INET, SOCK_DGRAM, 0);
	if (inputevent_socket < 0) perror("socket");

	bzero(&inputevent_server,sizeof(inputevent_server));
	inputevent_server.sin_family = AF_INET;
	inputevent_server.sin_addr.s_addr = inet_addr("127.0.0.1");
	inputevent_server.sin_port = htons(DEFAULT_PTR_KEY_PORT);
	length = sizeof(struct sockaddr_in);

	memset(temp_buffer,0,1024);
	//sprintf(temp_buffer,"~SHOW|send_event_to_app:%s \n",send_buffer);
//	sendMsgToGui(temp_buffer);
//	L("---send_event_to_app:%s---\n",send_buffer);
	n = sendto(inputevent_socket,send_buffer,strlen(send_buffer),0,(struct sockaddr *)&inputevent_server,length);
	if (n < 0) perror("Sendto");

	memset(temp_buffer,0,1024);
//	sprintf(temp_buffer,"~SHOW|send_event_to_app:length=%d \n",n);
//	sendMsgToGui(temp_buffer);
//	L("---send_event_to_app:%s---\n",temp_buffer);

	close(inputevent_socket);
	return 0;
}
...
void keyEvent(rfbBool down, rfbKeySym key, rfbClientPtr cl)
{
	...
	memset(send_event_buf,0,sizeof(send_event_buf));
	strcpy(send_event_buf,"~KEY|");
	...
	sprintf(temp, "%d|" ,down);
	strcat(send_event_buf,temp);

	memset(temp,0,sizeof(temp));
	sprintf(temp, "%d|" ,key);
	strcat(send_event_buf,temp);

	ret=send_event_to_app(send_event_buf);
	sprintf(temp_buffer,"~SHOW|send_event_to_app:%s \n",send_event_buf);
	sendMsgToGui(temp_buffer);
	L("---send_event_to_app:%s---\n",temp_buffer);
#else
	int sh = 0;
		...
		if ((code = keysym2scancode(down, key, cl,&sh,&alt)))
		{
			...
			if (key && down)
			{
				if (sh)
				{
					strcat(send_event_buf,"42|");//left shift
				}
				else
				{
					strcat(send_event_buf,"0|");
				}

				if (alt)
				{
					strcat(send_event_buf,"56|");//left alt
				}
				else
				{
					strcat(send_event_buf,"0|");
...
//	L("buttonMask=%d,leftClicked=%d \n",buttonMask,leftClicked);

	if((buttonMask & 1)&& leftClicked) {//left btn clicked and moving

		strcat(send_event_buf,"1|2|"); //left btn ACTION_MOVE
		send_event_to_app(send_event_buf);
	}
	...
	else if (buttonMask & 1)//left btn clicked
	{
		...
		strcat(send_event_buf,"1|0|"); //left btn ACTION_DOWN
		send_event_to_app(send_event_buf);
	}
	else if (leftClicked)//left btn released
	{
		leftClicked=0;
		strcat(send_event_buf,"1|1|"); //left btn ACTION_UP
		send_event_to_app(send_event_buf);
	}
	...
	{
		rightClicked=1;
		strcat(send_event_buf,"4|0|"); //right btn ACTION_DOWN
		send_event_to_app(send_event_buf);
	}
	else if (rightClicked)//right button released
	{
		rightClicked=0;
		strcat(send_event_buf,"4|1|");//right btn ACTION_UP
		send_event_to_app(send_event_buf);
	}
	...
	{
		middleClicked=1;
		strcat(send_event_buf,"2|0|"); //right btn ACTION_DOWN
		send_event_to_app(send_event_buf);
	}
	else if (middleClicked)// mid btn released
	{
		middleClicked=0;
		strcat(send_event_buf,"2|1|");// mid btn ACTION_UP
		send_event_to_app(send_event_buf);
	}
	if (buttonMask & 16)//mid btn up
	{
		rightClicked=1;
		strcat(send_event_buf,"16|2|"); //mid btn down for zoom out
		send_event_to_app(send_event_buf);
	}

	if (buttonMask & 8)//mid btn down
	{
		rightClicked=1;
		strcat(send_event_buf,"8|2|"); //mid btn down for zoom in
		send_event_to_app(send_event_buf);
	}
}
...
void cleanupInput()
{
	L("---cleanupInput inputevent_socket...---\n");
}
