...
volatile rfbBool bServerRun = FALSE;

void (*update_screen)(void)=NULL;

enum method_type {AUTO,SCREENCAP,FRAMEBUFFER,ADB,GRALLOC,FLINGER};
...
void initGrabberMethod()
{
	if (method == AUTO) {
	 ...
	 //   sendMsgToGui("~SHOW|initGrabberMethod\n");
		else if (initFlinger() != -1)
		{
		//	sendMsgToGui("~SHOW|initFlinger\n");
			method = FLINGER;
		}
		else if (initGralloc()!=-1)
		{
	//		sendMsgToGui("~SHOW|initGralloc\n");
			method = GRALLOC;
		}
		else if (initFB() != -1) {
	//		sendMsgToGui("~SHOW|initFB\n");
			method = FRAMEBUFFER;
		}
		else if (initADB() != -1) {
			method = ADB;
...
int startVncMain(int argc, char **argv){
	main(argc, argv);
}
int main(int argc, char **argv)
{
	...
	while (bServerRun) {
...
#else
		usec=(vncscr->deferUpdateTime+standby)*1;
#endif
		...
		if (repeaterHost != NULL)
		{
			...
			if (RepeaterGone == TRUE)
			{
				...
				RepeaterGone = FALSE;
				rfbLog("createRepeaterClient be called second !\n");
			//	sleep(30);
				repeater = createRepeaterClient();					
			}
		}
	...
	L("Vnc server main quit.");
	close_app();
}

int hasClientConnected(){
	L("hasClientConnected clientHead:%d\n", vncscr->clientHead);
	if (vncscr->clientHead != NULL)
	{
		L("hasClientConnected clientHead:%d\n", vncscr->clientHead);
		return 1;
	}
	L("hasClientConnected clientHead null\n");
	return 0;
}
