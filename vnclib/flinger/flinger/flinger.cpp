...
//#define GENERATE_BMP_FOR_TEST 1
...
PixelFormatInformation PixelFormatInfo;
...
#if defined(GENERATE_BMP_FOR_TEST)
#include <string.h>
#include <stdio.h>
...
int bmp_generator(int height,int width,unsigned char *data,int count)
{
	char file_name[128];
	memset(file_name,0,128);
	sprintf(file_name,"/data/data/screen_%d.bmp",count);
    ...
    //copy the data
    FILE *fp;
    if(!(fp=fopen(file_name,"wb"))) return 0;
...
extern "C" int init_flinger()
{
  ...
  //  errno = screenshotClient->update(display);
    errno = screenshotClient->update(SurfaceComposerClient::getBuiltInDisplay(0), Rect(), false);

    L("Screenshot client updated its display on init.,errno=%d\n",errno);
...
extern "C" unsigned int *checkfb_flinger()
{
   // screenshotClient->update(display);
	 screenshotClient->update(SurfaceComposerClient::getBuiltInDisplay(0), Rect(), false);
...
extern "C" unsigned int *readfb_flinger()
{
    ...
    if (s > w) {
        ...
        for (size_t y = 0; y < h; y++) {
			...
			if(count % 100 == 0)
			{
				bmp_generator(h,w,(unsigned char*)new_base,count);
			}
			count++;
		#endif
